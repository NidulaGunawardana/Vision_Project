...
class ZS6D:
    ...
    def __init__(
        ...
        self.extractor = PoseViTExtractor(
            model_type=self.model_type, stride=self.stride, device=self.device
        )
        ...
        try:
            for obj_id, template_labels in tqdm(self.templates_gt.items()):
                self.templates_desc[obj_id] = torch.cat(
                    [
                        torch.from_numpy(np.load(template_label["img_desc"])).unsqueeze(
                            0
                        )
                        for i, template_label in enumerate(template_labels)
                        if i % subset_templates == 0
                    ],
                    dim=0,
                )

                templates_gt_subset[obj_id] = [
                    template_label
                    for i, template_label in enumerate(template_labels)
                    if i % subset_templates == 0
                ]
    ...
    def get_pose(self, img, obj_id, mask, cam_K, bbox=None):
        try:
            ...
            img_crop, y_offset, x_offset = img_utils.make_quadratic_crop(
                np.array(img), bbox
            )
            ...
            matched_templates = utils.find_template_cpu(
                desc, self.templates_desc[obj_id], num_results=1
            )
            ...
            template = Image.open(
                self.templates_gt[obj_id][matched_templates[0][1]]["img_crop"]
            )
            ...
            with torch.no_grad():
                ...
                points1, points2, crop_pil, template_pil = (
                    self.extractor.find_correspondences_fastkmeans(
                        img_crop, template, num_pairs=20, load_size=crop_size
                    )
                )
                ...
                R_est, t_est = utils.get_pose_from_correspondences(
                    points1,
                    points2,
                    y_offset,
                    x_offset,
                ...
                return R_est, t_est, img_prep_np, img_crop_np
        except Exception as e:
